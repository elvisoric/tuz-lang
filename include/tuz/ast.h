...
struct Expr {
    ...
    Expr(ExprKind k, uint32_t ln, uint32_t col) 
...
struct Stmt {
    ...
    Stmt(StmtKind k, uint32_t ln, uint32_t col)
        : kind(k), line(ln), column(col) {}
...
struct IfStmt : Stmt {
    ...
    IfStmt(ExprPtr cond, StmtPtr then_br, StmtPtr else_br, uint32_t ln, uint32_t col)
        : Stmt(StmtKind::If, ln, col), condition(std::move(cond)), 
...
struct ForStmt : Stmt {
    ...
    ForStmt(std::string var, ExprPtr start, ExprPtr end, StmtPtr b, uint32_t ln, uint32_t col)
        : Stmt(StmtKind::For, ln, col), var_name(std::move(var)),
          range_start(std::move(start)), range_end(std::move(end)), body(std::move(b)) {}
};
...
struct FunctionDecl : Decl {
    ...
    FunctionDecl(std::string n, std::vector<Param> p, TypePtr ret, StmtPtr b, 
                 bool ext, uint32_t ln, uint32_t col)
