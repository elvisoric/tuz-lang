...
void CodeGenerator::visit(VariableExpr& expr) {
    ...
    // For now, always load
    llvm::Value* loaded = builder_->CreateLoad(convert_type(expr.type ? expr.type : get_int32_type()), var, expr.name);
    push_value(loaded);
}
...
void CodeGenerator::visit(ForStmt& stmt) {
    ...
    // Condition
    builder_->SetInsertPoint(cond_bb);
    llvm::Value* current = builder_->CreateLoad(llvm::Type::getInt32Ty(*context_), loop_var, stmt.var_name);
    ...
    llvm::Value* step_val = builder_->CreateLoad(llvm::Type::getInt32Ty(*context_), loop_var, stmt.var_name);
    llvm::Value* next_val = builder_->CreateAdd(step_val, llvm::ConstantInt::get(llvm::Type::getInt32Ty(*context_), 1), "next");
...
void CodeGenerator::declare_function(FunctionDecl& decl) {
    ...
    llvm::Function* function = llvm::Function::Create(func_type, 
        llvm::Function::ExternalLinkage, decl.name, module_.get());
...
llvm::Value* CodeGenerator::codegen_binary_op(BinaryOp op, llvm::Value* left, llvm::Value* right, TypePtr result_type) {
...
llvm::Value* CodeGenerator::get_element_ptr(llvm::Value* ptr, const std::vector<llvm::Value*>& indices, llvm::Type* elem_type) {
