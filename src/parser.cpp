...
DeclPtr Parser::parse_extern_decl() {
    ...
    return std::make_shared<FunctionDecl>(name, std::move(params), return_type, 
                                          nullptr, true, line, col);
}
...
DeclPtr Parser::parse_function_decl() {
    ...
    return std::make_shared<FunctionDecl>(name, std::move(params), return_type, 
                                          std::move(body), is_extern, line, col);
}
...
StmtPtr Parser::parse_for_stmt() {
    ...
    return std::make_shared<ForStmt>(var_name, std::move(range_start), 
                                     std::move(range_end), std::move(body), line, col);
}
...
ExprPtr Parser::parse_or() {
    ...
    while (match(TokenType::OR)) {
        ...
        expr = std::make_shared<BinaryOpExpr>(BinaryOp::Or, std::move(expr), std::move(right), line, col);
    }
...
ExprPtr Parser::parse_and() {
    ...
    while (match(TokenType::AND)) {
        ...
        expr = std::make_shared<BinaryOpExpr>(BinaryOp::And, std::move(expr), std::move(right), line, col);
    }
...
ExprPtr Parser::parse_equality() {
    ...
    while (true) {
        ...
        if (match(TokenType::EQ)) {
            ...
            expr = std::make_shared<BinaryOpExpr>(BinaryOp::Eq, std::move(expr), std::move(right), line, col);
        } else if (match(TokenType::NEQ)) {
            ExprPtr right = parse_comparison();
            expr = std::make_shared<BinaryOpExpr>(BinaryOp::Neq, std::move(expr), std::move(right), line, col);
        } else {
            break;
        }
    }
...
ExprPtr Parser::parse_comparison() {
    ...
    while (true) {
        ...
        if (match(TokenType::LT)) {
            ...
            expr = std::make_shared<BinaryOpExpr>(BinaryOp::Lt, std::move(expr), std::move(right), line, col);
        } else if (match(TokenType::GT)) {
            ExprPtr right = parse_term();
            expr = std::make_shared<BinaryOpExpr>(BinaryOp::Gt, std::move(expr), std::move(right), line, col);
        } else if (match(TokenType::LEQ)) {
            ExprPtr right = parse_term();
            expr = std::make_shared<BinaryOpExpr>(BinaryOp::Leq, std::move(expr), std::move(right), line, col);
        } else if (match(TokenType::GEQ)) {
            ExprPtr right = parse_term();
            expr = std::make_shared<BinaryOpExpr>(BinaryOp::Geq, std::move(expr), std::move(right), line, col);
        } else {
            break;
        }
    }
...
ExprPtr Parser::parse_term() {
    ...
    while (true) {
        ...
        if (match(TokenType::PLUS)) {
            ...
            expr = std::make_shared<BinaryOpExpr>(BinaryOp::Add, std::move(expr), std::move(right), line, col);
        } else if (match(TokenType::MINUS)) {
            ExprPtr right = parse_factor();
            expr = std::make_shared<BinaryOpExpr>(BinaryOp::Sub, std::move(expr), std::move(right), line, col);
        } else {
            break;
        }
    }
...
ExprPtr Parser::parse_factor() {
    ...
    while (true) {
        ...
        if (match(TokenType::STAR)) {
            ...
            expr = std::make_shared<BinaryOpExpr>(BinaryOp::Mul, std::move(expr), std::move(right), line, col);
        } else if (match(TokenType::SLASH)) {
            ExprPtr right = parse_unary();
            expr = std::make_shared<BinaryOpExpr>(BinaryOp::Div, std::move(expr), std::move(right), line, col);
        } else if (match(TokenType::PERCENT)) {
            ExprPtr right = parse_unary();
            expr = std::make_shared<BinaryOpExpr>(BinaryOp::Mod, std::move(expr), std::move(right), line, col);
        } else {
            break;
        }
    }
